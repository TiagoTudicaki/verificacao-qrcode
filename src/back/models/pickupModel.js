...
const pickupModel = {
  async create(orderNumber, clientToken, equipmentToken) {
    const sql =
      "INSERT INTO pickups (order_number, client_token, equipment_token)VALUES(?, ?, ?)";
  ...
  async update(id, newStatus, checkedAt, confirmationMethod) {
    const [confirmedPickup] = await db.query(
      "UPDATE pickups SET status = ?, checked_at = ?, confirmation_method = ? WHERE id = ?",
      [newStatus, checkedAt, confirmationMethod, id],
    );
